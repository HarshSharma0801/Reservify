...
const Display = (props) => {
  ...
  return (
    ...
    <h1 className="text-2xl md:text-4xl flex flex-start px-5 text-center text-primary">
             <Typewriter
            words={['Book It, Live It, Love It!', 'Your Next Adventure Awaits – Reserve Today!', 'Turning Dreams into Reservations', 'Reserve, Relax, Rejoice!']}
            loop={0}
            cursor
            typeSpeed={100}
            deleteSpeed={50}
                ...
                <div className="p-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <p className="flex font-normal text-lg text-gray-700 dark:text-gray-400">
                      {Reserve.rating}<img className="w-6 h-6" src="https://img.icons8.com/emoji/48/star-emoji.png" alt="star-emoji"/>
                  ...
                  <p className=" font-normal text-base text-black dark:text-gray-400">
                     <span className="font-bold  text-xl">${Reserve.price}</span> {Reserve.category=='accommodation' ? 'night' : 'table' }
                  </p>
                  <p>
                    <Link to={`/Reserve/${Reserve._id}`} className='text-base text-primary hover:underline'>view details</Link>
                  </p>
                  </div>
